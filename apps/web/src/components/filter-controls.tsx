...
export function FilterControls({ state, onApply }: Readonly<{ state: WorkspaceState; onApply: (patch: Partial<WorkspaceState>) => void }>) {
  ...
  return (
    <form className="filter-controls" onSubmit={(event) => { event.preventDefault(); onApply({ ...values, offset: undefined, selected: undefined }); }}>
      <label>Search activity<input aria-label="Search activity" defaultValue={state.query} onChange={(event) => update("query", event.target.value)} /></label>
