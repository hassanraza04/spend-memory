...
export function ComparisonView({ accounts, account, currency, scope, hasWorkspace, hasPreviousPeriod, comparison, loadError, onScopeChange }: Readonly<ComparisonViewProps>) {
  ...
  const heading = <><p className="eyebrow">Compare</p><h1 id="comparison-title">What changed?</h1>{scope && <p className="scope-note">Scope: {scope}</p>}{controls}</>;
  ...
  const topContribution = comparison.contributions[0];
  return <section className="record-view" aria-labelledby="comparison-title">{heading}<p className="intro">{comparison.text}</p>{topContribution && <p className="scope-note">Top contributor: {topContribution.label} ({formatMoney(topContribution.amount_minor, activeCurrency)}).</p>}<dl className="comparison-totals" aria-label="Exact comparison totals"><div><dt>Earlier net</dt><dd>{formatMoney(comparison.before_net_amount_minor, activeCurrency)}</dd></div><div><dt>Later net</dt><dd>{formatMoney(comparison.after_net_amount_minor, activeCurrency)}</dd></div><div><dt>Exact change</dt><dd>{formatMoney(comparison.difference_net_amount_minor, activeCurrency)}</dd></div></dl><RecordChart title="Waterfall text alternative" values={comparison.contributions.map((contribution) => ({ label: contribution.label, value: formatMoney(contribution.amount_minor, activeCurrency) }))} /><table className="contribution-table"><caption>Exact contribution evidence</caption><thead><tr><th scope="col">Contributor</th><th scope="col">Change</th><th scope="col">Evidence rows</th></tr></thead><tbody>{comparison.contributions.map((contribution) => <tr key={contribution.label}><th scope="row">{contribution.label}</th><td>{formatMoney(contribution.amount_minor, activeCurrency)}</td><td>{contribution.before_transaction_ids.length + contribution.after_transaction_ids.length}</td></tr>)}</tbody></table></section>;
}
